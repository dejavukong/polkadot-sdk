...
pub fn new_full_base<N: NetworkBackend<Block, <Block as BlockT>::Hash>>(
	...
	// 	&sc_consensus_manual_seal::BabeBlockImport<
	// 		Block,
	// 		FullClient,
	// 		FullBeefyBlockImport<FullGrandpaBlockImport>,
	// 	>,
	...
	let sc_service::PartialComponents {
		...
		transaction_pool,
		other:
			(rpc_builder, mut telemetry, statement_store, mixnet_api_backend),
	...
	let (network, system_rpc_tx, tx_handler_controller, sync_service) =
		sc_service::build_network(sc_service::BuildNetworkParams {
			...
			warp_sync_config: None, // Some(WarpSyncConfig::WithProvider(warp_sync)),
			block_relay: None,
			metrics,
		})?;
	...
	if let sc_service::config::Role::Authority { .. } = &role {
		...
		let seal = sc_consensus_manual_seal::run_instant_seal(
